...
def visualize_airline_statistics(df, airline_data, airline_statistics):
    ...
    for i in range(len(abbr_companies)):
        comp_handler.append(mpatches.Patch(color=colors[i],
                                           label=airline_statistics.index[i] + ': ' +
    ...
    ax3 = sns.stripplot(y="OP_CARRIER", x="DEP_DELAY", size=4, palette=colors,
                        data=df2, linewidth=0.5, jitter=True)
    plt.setp(ax3.get_xticklabels(), fontsize=14)
    plt.setp(ax3.get_yticklabels(), fontsize=14)
    ax3.set_xticklabels(['{:2.0f}h{:2.0f}m'.format(*[int(y) for y in divmod(x, 60)])
                         for x in ax3.get_xticks()])
    plt.xlabel('Departure delay', fontsize=5, bbox={'facecolor': 'midnightblue', 'pad': 5},
               color='w', labelpad=20)
